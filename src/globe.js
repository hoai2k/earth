...
(function (root) {
  ...
  function Globe(canvas, mesh) {
    ...
    // + pitch clamped to [-90,90] (tilt toward a pole; no momentum, never rolls).
    // earth.quaternion = pitch(about screen-X) * yaw(about world-Y).
    const X_AXIS = new THREE.Vector3(1, 0, 0);
    let yaw = 0, pitch = 0, yawVel = 0;
    const PITCH_LIM = Math.PI / 2;
    ...
    function dragSpin(dx, dy) {   // grabbed surface follows the cursor
      const sp = 0.006;
      yaw += dx * sp;
      pitch = Math.max(-PITCH_LIM, Math.min(PITCH_LIM, pitch + dy * sp));
    ...
    function faceDir(dir) {
      ...
      pitch = Math.atan2(F.z, F.y) - Math.atan2(dp.z, dp.y);
